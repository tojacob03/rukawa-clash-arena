-- Drop and recreate authenticate_client_secure function to fix all issues
DROP FUNCTION IF EXISTS public.authenticate_client_secure(text, inet, text);

-- Create the function with proper unique column names
...
BEGIN
  ...
  -- Log the attempt
  INSERT INTO public.client_login_attempts (login_code, ip_address, attempted_at, success)
  VALUES (login_code_param, ip_address_param, now(), found_client.id IS NOT NULL AND found_client.is_active);

  -- Check if client exists and is active
  IF found_client.id IS NULL OR found_client.is_active = false THEN
  ...
  DELETE FROM public.client_sessions WHERE client_sessions.client_id = found_client.id;
