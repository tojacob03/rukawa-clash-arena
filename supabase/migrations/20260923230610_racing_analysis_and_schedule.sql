...
begin
  ...
  deg as (
    select jsonb_agg(jsonb_build_object(
             'compound', compound, 'sec_per_lap', round(avg(slope)::numeric, 3),
             'stints', count(*), 'laps', sum(n)) order by compound) as arr
...
begin
  ...
  deg as (
    select jsonb_agg(jsonb_build_object('compound', compound, 'sec_per_lap', round(avg(s), 3), 'races', count(*))
                     order by compound) as arr
    from deg_rows group by compound
  ),
  finisher_stops as (
    select res.session_key, res.driver_number,
  ...
  stops_per_race as (
    select jsonb_agg(jsonb_build_object('session_key', f.session_key, 'circuit', y.circuit_short_name,
                                        'avg_stops', round(avg_n, 2)) order by y.date_start) as arr
